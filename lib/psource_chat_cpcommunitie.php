<?php
function psource_chat_cpcommunitie_init() {
	if ( class_exists( 'CPC_Group_Extension' ) ) {
		include_once( 'cpcommunitie/psource_chat_cpcommunitie_group.php' );
	}

	if ( class_exists( 'PSOURCE_Chat_CPCommunitie' ) ) {
		cpc_register_group_extension( 'PSOURCE_Chat_CPCommunitie' );
	}
}

add_action( 'cpc_include', 'psource_chat_cpcommunitie_init' );

function psource_chat_buddypress_friends_list() {
	global $cpc, $members_template, $psource_chat;
	...
	if ( ( ( isset( $members_template->member->is_friend ) ) && ( $members_template->member->is_friend == true ) ) || ( $cpc->loggedin_user->is_site_admin == true ) ) {

		$content = '';

		$content .= '<div id="psource-chat-now-button-' . $members_template->member->id . '" class="psource-chat-now-button">';
		$friends_status = psource_chat_get_friends_status( $cpc->loggedin_user->id, $members_template->member->id );
...
add_action( 'cpc_member_header_actions', 'psource_chat_cpcommunitie_member_header_actions', 30 );
add_action( 'cpc_directory_members_actions', 'psource_chat_cpcommunitie_friends_list' );
add_action( 'cpc_group_members_list_item_action', 'psource_chat_cpcommunitie_friends_list' );

function psource_chat_cpcommunitie_member_header_actions() {
	global $cpc, $members_template, $psource_chat, $current_user;

	if ( $cpc->loggedin_user->id === cpc_displayed_user_id() ) {
		return;
	}

	//if (!is_object('BP_Friends_Friendship')) return;
	if ( ! cpc_is_active( 'friends' ) ) {
		return;
	}

	if ( ! class_exists( 'CPC_Friends_Friendship' ) ) {
		return

			$is_friend_ret = CPC_Friends_Friendship::check_is_friend( cpc_loggedin_user_id(), cpc_displayed_user_id() );
	}
	...
	$psource_chat->_chat_plugin_settings['blocked_urls']['front'] = false;
	if ( ( CPC_Friends_Friendship::check_is_friend( cpc_loggedin_user_id(), cpc_displayed_user_id() ) == 'is_friend' )
	     || ( $cpc->loggedin_user->is_site_admin == true )
	) {

		$content = '';

		$content .= '<div id="psource-chat-now-button-' . bp_displayed_user_id() . '" class="generic-button psource-chat-now-button">';
		$friends_status = psource_chat_get_friends_status( $cpc->loggedin_user->id, cpc_displayed_user_id() );
		...
		$friend_data = psource_chat_get_chat_status_data( cpc_displayed_user_id(), $friends_status );
...
function psource_chat_cpcommunitie_settings_setup_nav() {
	global $psource_chat;
	...
	if ( cpc_displayed_user_domain() ) {
		$user_domain = cpc_displayed_user_domain();
	} elseif ( cpc_loggedin_user_domain() ) {
		$user_domain = cpc_loggedin_user_domain();
	} else {
		return;
	}
	...
	$sub_nav = array(
		...
		'position'        => 30,
		'user_has_access' => cpc_core_can_edit_settings()
	);
	cpc_core_new_subnav_item( $sub_nav );
}

add_action( 'cpc_settings_setup_nav', 'psource_chat_cpcommunitie_settings_setup_nav' );

function psource_chat_settings_screen_chat_proc() {
	global $psource_chat, $current_user, $cpc;
	...
	if ( isset( $_POST['psource_chat_user_settings'] ) ) {
		...
		if ( isset( $_POST['psource_chat_user_settings']['chat_user_status'] ) ) {
			...
			if ( isset( $psource_chat->_chat_options['user-statuses'][ $chat_user_status ] ) ) {
				psource_chat_update_user_status( $cpc->displayed_user->id, $chat_user_status );
			}
			unset( $_POST['psource_chat_user_settings']['chat_user_status'] );
		}

		$user_meta = get_user_meta( $cpc->displayed_user->id, 'psource-chat-user', true );
		if ( ! $user_meta ) {
			$user_meta = array();
		}
	...
	add_action( 'cpc_template_title', 'psource_chat_settings_show_screen_title' );
	add_action( 'cpc_template_content', 'psource_chat_settings_show_screen_content' );
	bp_core_load_template( apply_filters( 'cpc_core_template_plugin', 'members/single/plugins' ) );
...
function psource_chat_settings_show_screen_content() {
	...
	$chat_user_status = psource_chat_get_user_status( $cpc->displayed_user->id );
	if ( isset( $psource_chat->_chat_options['user-statuses'][ $chat_user_status ] ) ) {
